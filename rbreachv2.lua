...
-- Initialize items (with distance filtering)
allItems = getUniqueItems()

-- Create dropdown and store reference for updates
...
-- Auto-refresh every 3 seconds and update dropdown dynamically
task.spawn(function()
    while true do
        task.wait(3)
...
-- Incremental teleport function to bypass anti-cheat
-- Includes crash protection and physics-safe delays
local function smoothTeleport(targetPos, stepSize)
    local success, result = pcall(function()
        ...
        -- If distance is small, teleport directly
        if distance <= stepSize then
            myRoot.CFrame = CFrame.new(targetPos)
            return true
        end
        
        -- Calculate number of steps with a MAX LIMIT to prevent crashes
        local steps = math.ceil(distance / stepSize)
        local MAX_STEPS = 15  -- Prevent excessive teleports that could crash Roblox
        
        if steps > MAX_STEPS then
            -- If too many steps, increase step size to fit within limit
            stepSize = math.ceil(distance / MAX_STEPS)
            steps = MAX_STEPS
            print("[!] Distance too long, using larger steps:", stepSize, "studs per step")
        end
        
        local direction = (targetPos - startPos).Unit
        
        -- Teleport in increments with crash protection
        for i = 1, steps do
            -- Double-check character still exists
            if not LocalPlayer.Character or not myRoot or not myRoot.Parent then 
                warn("[!] Character lost during teleport at step", i)
                return false 
            end
            ...
            if not tpSuccess then
                warn("[!] Teleport failed at step", i)
                return false
            end
            
            -- Longer wait between steps to prevent physics overload
            -- Also wait for physics update
            RunService.Heartbeat:Wait()
            local waitTime = 0.1 + (math.random() * 0.05)  -- Increased from 0.05
            task.wait(waitTime)
            
            -- Every 5 steps, take a slightly longer break for stability
            if i % 5 == 0 then
                task.wait(0.1)
            end
        end
    ...
    if not success then
        warn("[!] smoothTeleport error:", result)
        return false
    end
...
-- Cooldown system to prevent crashes from rapid teleportation
...
local function canTeleport()
    ...
    if currentTime - lastTeleportTime < TELEPORT_COOLDOWN then
        local remaining = math.ceil(TELEPORT_COOLDOWN - (currentTime - lastTeleportTime))
        UILib:CreateNotification({Text = "Wait " .. remaining .. "s before teleporting again!", Duration = 2})
        return false
    end
...
UILib:CreateButton(PlayerPanel, {
    Text = "Get Item (Instant - 200 Studs)",
    Callback = function()
        ...
        local success, err = pcall(function()
            if not selectedItem or selectedItem == "Scanning..." or selectedItem == "No items found" or selectedItem == "No ItemSpawns folder" then
                UILib:CreateNotification({Text = "Please select a valid item!", Duration = 3})
                return
            end
            if not LocalPlayer.Character then
                UILib:CreateNotification({Text = "No character found!", Duration = 3})
                return
            end
            local myRoot = getRoot(LocalPlayer.Character)
            if not myRoot then
                UILib:CreateNotification({Text = "No HumanoidRootPart!", Duration = 3})
                return
            end
            
            local itemsFolder = Workspace:FindFirstChild("ItemSpawns")
            if not itemsFolder then
                UILib:CreateNotification({Text = "ItemSpawns folder not found!", Duration = 3})
                return
            end
            ...
            if targetItem and targetItemModel then
                ...
                print("[*] Item is", math.floor(distance), "studs away - grabbing...")
                UILib:CreateNotification({Text = "Getting item...", Duration = 2})
                ...
                if proximityPrompt then
                    ...
                    if clickDetector then
                        ...
                        UILib:CreateNotification({Text = "No prompt/detector found!", Duration = 3})
                    end
                end
                ...
                if myRoot and myRoot.Parent then
                    ...
                    UILib:CreateNotification({Text = "Item grabbed successfully!", Duration = 3})
                    print("[+] Returned to original position")
                end
            else
                UILib:CreateNotification({Text = "Item not found on map!", Duration = 3})
                warn("[!] Item '" .. selectedItem .. "' not found on map")
            end
        end)
        
        if not success then
            UILib:CreateNotification({Text = "Error: " .. tostring(err), Duration = 3})
            warn("[!] Error grabbing item:", err)
        end
    end
})
UILib:CreateButton(PlayerPanel, {
    Text = "Escape (Anti-Cheat Bypass)",
    Callback = function()
        if not canTeleport() then return end
        ...
        local success, err = pcall(function()
            ...
            if distance > 1000 then
                UILib:CreateNotification({Text = "Escape too far: " .. math.floor(distance) .. " studs!", Duration = 3})
                return
            end
            
            print("[*] Escape is", math.floor(distance), "studs away")
            UILib:CreateNotification({Text = "Escaping... " .. math.floor(distance) .. " studs", Duration = 2})
            
            -- Teleport incrementally to escape (50 studs per step)
            local teleportSuccess = smoothTeleport(escapePos, 50)
            
            if not teleportSuccess then
                UILib:CreateNotification({Text = "Teleport failed!", Duration = 3})
                return
            end
            
            task.wait(0.3)
            print("[+] Reached escape location!")
            UILib:CreateNotification({Text = "At escape! Returning...", Duration = 2})
            
            -- Return to original position incrementally
            if myRoot and myRoot.Parent then
                myRoot.Velocity = Vector3.new(0, 0, 0)
                myRoot.RotVelocity = Vector3.new(0, 0, 0)
                local humanoid = getHumanoid(LocalPlayer.Character)
                ...
                smoothTeleport(originalPos.Position, 50)
                ...
                UILib:CreateNotification({Text = "Escape completed!", Duration = 3})
                print("[+] Returned to original position")
            end
        end)
        if not success then
...
UILib:CreateButton(PlayerPanel, {
    Text = "Get Item AND Escape (Bypass)",
    Callback = function()
        if not canTeleport() then return end
        ...
        local success, err = pcall(function()
            ...
            if not targetItem or not targetItemModel then
                UILib:CreateNotification({Text = "Item not found on map!", Duration = 3})
                return
            end
            ...
            local originalPos = myRoot.CFrame
            local itemDistance = (targetItem.Position - myRoot.Position).Magnitude
            
            if itemDistance > 200 then
                UILib:CreateNotification({Text = "Item too far: " .. math.floor(itemDistance) .. " studs!", Duration = 3})
                return
            end
            
            print("[*] Starting Get Item AND Escape sequence...")
            UILib:CreateNotification({Text = "Step 1: Getting item...", Duration = 2})
            
            -- STEP 1: Teleport to item incrementally
            local itemTargetPos = targetItem.Position + Vector3.new(0, 2, 0)
            local teleportSuccess = smoothTeleport(itemTargetPos, 50)
            
            if not teleportSuccess then
                UILib:CreateNotification({Text = "Failed to reach item!", Duration = 3})
                return
            end
            ...
            if proximityPrompt then
                ...
                if clickDetector then
                    ...
                    UILib:CreateNotification({Text = "Could not grab item!", Duration = 3})
                end
            end
            
            -- Verify character still exists
            if not myRoot or not myRoot.Parent then
                UILib:CreateNotification({Text = "Character lost during grab!", Duration = 3})
                return
            end
            
            -- STEP 2: Teleport to escape
            UILib:CreateNotification({Text = "Step 2: Escaping...", Duration = 2})
            print("[*] Teleporting to escape...")
            
            local escapePos = Vector3.new(1112.26, 566.00, 56.86)
            local escapeDistance = (escapePos - myRoot.Position).Magnitude
            
            if escapeDistance > 1000 then
                UILib:CreateNotification({Text = "Escape too far: " .. math.floor(escapeDistance) .. " studs!", Duration = 3})
                -- Still return to original position
                smoothTeleport(originalPos.Position, 50)
                task.wait(0.1)
                if myRoot and myRoot.Parent then
                    myRoot.CFrame = originalPos
            ...
            -- Teleport to escape incrementally
            teleportSuccess = smoothTeleport(escapePos, 50)
            
            if not teleportSuccess then
                UILib:CreateNotification({Text = "Failed to reach escape!", Duration = 3})
                return
            end
            
            task.wait(0.3)
            print("[+] Reached escape location!")
            UILib:CreateNotification({Text = "Step 3: Returning...", Duration = 2})
            ...
            if myRoot and myRoot.Parent then
                ...
                UILib:CreateNotification({Text = "✓ Item grabbed & escaped!", Duration = 4})
                print("[+] Successfully completed: Get Item AND Escape")
            end
        end)
        
        if not success then
            UILib:CreateNotification({Text = "Error: " .. tostring(err), Duration = 3})
            warn("[!] Error in Get Item AND Escape:", err)
        end
    end
})
