...
rapidFireConnection = RunService.Heartbeat:Connect(function()
    if Config.RapidFire then
        -- Only search for weapon state if not cached
        if not weaponStateCached then
            weaponStateTable = findWeaponState()
        end
        
        if weaponStateTable then
            pcall(function()
                -- Keep cycled = true to bypass fire rate cooldown
                weaponStateTable.cycled = true
                
                -- Boost fire rate to maximum
                ...
                if Config.InfiniteAmmo then
                    ...
                    if tool then
                        ...
                        if ammo then
                            ...
                            if mag then
                                -- Just set to high number, don't check MaxValue
                                mag.Value = 999
                            end
                        end
                    end
...
-- Silent Aim - hookfunction implementation
local rapidFireActive = false
...
-- Hook BridgeNet2 fireWeapon specifically
if hookfunction and getgc then
    task.spawn(function()
        task.wait(3) -- Wait for weapon system to fully load
        
        warn("[Silent Aim] Searching for fireWeapon bridge...")
        
        -- Search for the v_u4 bridges table
        for _, obj in pairs(getgc(true)) do
            if type(obj) == "table" then
                -- Look for table that has repSound, fireWeapon, repHit bridges
                local hasRepSound = rawget(obj, "repSound")
                local hasFireWeapon = rawget(obj, "fireWeapon")
                local hasRepHit = rawget(obj, "repHit")
                
                if hasRepSound and hasFireWeapon and hasRepHit then
                    warn("[Silent Aim] Found bridges table!")
                    
                    -- Now hook the fireWeapon bridge's Fire method
                    local fireWeaponBridge = rawget(obj, "fireWeapon")
                    if fireWeaponBridge and rawget(fireWeaponBridge, "Fire") then
                        warn("[Silent Aim] Found fireWeapon.Fire!")
                        
                        local success = pcall(function()
                            local originalFire = hookfunction(fireWeaponBridge.Fire, function(...)
                                local args = {...}
                                local data = args[2]
                                
                                warn("[Silent Aim] fireWeapon:Fire called!")
                                ...
                                -- Modify mouseHit if silent aim active
                                if Config.SilentAim and rapidFireActive and data and type(data) == "table" and data.mouseHit then
                                    warn("[Silent Aim] Has mouseHit!")
                                    local target = getClosestPlayer()
                                    if target and target.Character then
                                        local head = _getHead(target.Character)
                                        if head then
                                            ...
                                            if onScreen then
                                                ...
                                                if distance <= Config.SilentAimFOV then
                                                    ...
                                                    if not rayResult then
                                                        warn("[Silent Aim] ✅ Modifying mouseHit!")
                                                        data.mouseHit = head.CFrame
                                                    else
                                                        warn("[Silent Aim] ⚠️ Wall blocking")
                                                    end
                                                end
                                            end
                                        end
                        ...
                        if success then
                            UILib:CreateNotification({
                                Text = "✅ fireWeapon hooked!",
                                Duration = 3
                            })
                            warn("[Silent Aim] ✅ Successfully hooked fireWeapon!")
                        else
                            warn("[Silent Aim] ❌ Failed to hook fireWeapon")
                        end
                    end
                    break
                end
    ...
    warn("[Silent Aim] hookfunction or getgc not supported")
end
