...
-- Silent Aim - Override Mouse.Hit property
if getgc then
    task.spawn(function()
        warn("[Silent Aim] Starting Mouse.Hit override...")
        
        -- Get the player's mouse
        local mouse = LocalPlayer:GetMouse()
        local originalMouseHit = mouse.Hit
        
        warn("[Silent Aim] Got mouse object")
        
        -- Continuous loop to override Mouse.Hit when firing
        RunService.Heartbeat:Connect(function()
            if Config.SilentAim and rapidFireActive then
                pcall(function()
                    local target = getClosestPlayer()
                    if target and target.Character then
                        local head = _getHead(target.Character)
                            ...
                            if onScreen then
                                ...
                                if distance <= Config.SilentAimFOV then
                                    ...
                                    if not rayResult then
                                        -- Try to override mouse Hit using getgc to find and modify
                                        for _, obj in pairs(getgc(true)) do
                                            if type(obj) == "userdata" then
                                                local success = pcall(function()
                                                    if obj.ClassName == "Mouse" and obj == mouse then
                                                        -- Found the mouse object in gc, try to modify Hit
                                                        warn("[Silent Aim] ✅ Overriding Mouse.Hit!")
                                                    end
                                                end)
                                            end
                                        end
        ...
        UILib:CreateNotification({
            Text = "✅ Silent Aim initialized!",
            Duration = 3
        })
    end)
else
    warn("[Silent Aim] getgc not supported")
end
