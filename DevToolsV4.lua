...
-- ═══════════════════════════════════════════════════════════════
-- FREECAM SYSTEM (Press V to toggle)
-- ═══════════════════════════════════════════════════════════════
    ...
    Callback = function(state)
        ...
        if FreecamMode then
            ...
            FreecamConnection = RunService.RenderStepped:Connect(function(dt)
                if not FreecamMode then return end
                ...
                if FreecamKeys.w then
                    direction = direction + (Camera.CFrame.LookVector * FreecamSpeed * dt)
                end
                if FreecamKeys.s then
                    direction = direction - (Camera.CFrame.LookVector * FreecamSpeed * dt)
                end
                if FreecamKeys.a then
                    direction = direction - (Camera.CFrame.RightVector * FreecamSpeed * dt)
                end
                if FreecamKeys.d then
                    direction = direction + (Camera.CFrame.RightVector * FreecamSpeed * dt)
                end
                if FreecamKeys.space then
                    direction = direction + (Vector3.new(0, 1, 0) * FreecamSpeed * dt)
                end
                if FreecamKeys.shift then
                    direction = direction - (Vector3.new(0, 1, 0) * FreecamSpeed * dt)
                end
            ...
            UILib:CreateNotification({Text = "Freecam Enabled (WASD + Space/Shift)", Duration = 3})
        else
            -- Disable freecam
            ...
            UILib:CreateNotification({Text = "Freecam Disabled", Duration = 2})
        end
    end
})

-- Freecam input handling
local freecamInputConn = UserInputService.InputBegan:Connect(function(input, gameProcessed)
    if gameProcessed and not FreecamMode then return end
    
    -- Toggle freecam with V key
...
table.insert(_G.DevTools.Connections, freecamInputConn)

local freecamInputEndConn = UserInputService.InputEnded:Connect(function(input)
    local key = input.KeyCode.Name:lower()
    if FreecamKeys[key] ~= nil then
...
table.insert(_G.DevTools.Connections, freecamInputEndConn)

UILib:CreateSlider(UtilsPanel, {
    Text = "Freecam Speed",
    Min = 10,
    Max = 200,
    Default = 50,
...
local NetworkPanel = UILib:CreatePanel(Window, {
    Name = "Network",
...
UILib:CreateToggle(NetworkPanel, {
    Label = "Monitor Network Traffic",
    Default = false,
    Callback = function(value)
        MonitorEnabled = value
        
        if value then
            UILib:CreateNotification({Text = "Network monitor started!", Duration = 3})
            ...
            task.spawn(function()
                local success = pcall(function()
                    ...
                    local bridges = {
                        "RoundHandler", "__fireWeapon", "__repHit", "repReload",
                        "repSound", "equipWeapon", "grenadeThrow", "serverTracer"
                    }
                    
                    for _, name in pairs(bridges) do
                        if not HookedBridges[name] then
                            task.spawn(function()
                                local ok, bridge = pcall(function()
                                    local BN2 = require(RS.Assets.Modules.BridgeNet2)
                                    return BN2.ClientBridge(name)
                                end)
                                        ...
                                        bridge:OutboundMiddleware({
                                            function(data)
                                                if MonitorEnabled then
                                                    ...
                                                    print(string.format("🔴 [%s → Server] %s", name, json))
                                        ...
                                        bridge:InboundMiddleware({
                                            function(data)
                                                if MonitorEnabled then
                                                    ...
                                                    -- Capture weapon stats
                                                    if name == "equipWeapon" and data.wepStats then
                                                        local weaponName = data.tool and data.tool.Name or "Unknown"
                                                        WeaponStatsCache[weaponName] = data.wepStats
                                                        print("📊 Captured stats for:", weaponName)
                                                    end
                                                end
                                                return data
                                            end
            ...
            UILib:CreateNotification({Text = "Monitor stopped", Duration = 2})
        end
    end
})

UILib:CreateButton(NetworkPanel, {
    Text = "Clear Traffic Log",
    Callback = function()
        TrafficLog = {}
        UILib:CreateNotification({Text = "Log cleared!", Duration = 2})
    end
})

UILib:CreateButton(NetworkPanel, {
    Text = "Export Traffic to File",
    Callback = function()
        if not writefile then
            UILib:CreateNotification({Text = "writefile not available!", Duration = 3})
            return
        end
        
        local HttpService = game:GetService("HttpService")
        local filename = "network_traffic_" .. os.time() .. ".json"
        writefile(filename, HttpService:JSONEncode(TrafficLog))
        
        UILib:CreateNotification({
            Text = "Exported " .. #TrafficLog .. " events to " .. filename,
            Duration = 5
        })
    end
})
...
UILib:CreateButton(NetworkPanel, {
    ...
    Callback = function()
        ...
        local HttpService = game:GetService("HttpService")
        local filename = "weapon_stats_" .. os.time() .. ".json"
        writefile(filename, HttpService:JSONEncode(WeaponStatsCache))
        
        local count = 0
        for _ in pairs(WeaponStatsCache) do count = count + 1 end
        
        UILib:CreateNotification({
            Text = "Exported " .. count .. " weapon stats to " .. filename,
            Duration = 5
        })
    end
})
...
UILib:CreateButton(NetworkPanel, {
    Text = "Print Weapon Stats",
    Callback = function()
        print("\n=== WEAPON STATS ===")
        for weaponName, stats in pairs(WeaponStatsCache) do
            print(string.format("\n%s:", weaponName))
            print("  Damage:", stats.BodyDamage or "N/A")
            print("  Headshot:", stats.Damage and stats.Damage.Headshot or "N/A")
            print("  Fire Rate:", stats.FireRate or "N/A")
            print("  Spread:", stats.Spread or "N/A")
            print("  Mag Size:", stats.Mag or "N/A")
        end
        print("====================\n")
    end
})
