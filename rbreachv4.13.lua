        ...
        pcall(function()
            ...
            if Config.RapidFire then
                if stats.FireRate then
                    ...
                    -- 200 is ~3x faster than default (600) but subtle enough to avoid detection
                    stats.FireRate = 1500
                end
            ...
            if Config.DamageMultiplier ~= 1 then
                if stats.Damage then
                    stats.Damage = stats.Damage * Config.DamageMultiplier
                end
                if stats.HeadshotMultiplier then
                    stats.HeadshotMultiplier = stats.HeadshotMultiplier * Config.DamageMultiplier
                end
            end
        end)
    end
        ...
        task.spawn(function()
            ...
            local success, err = _pcall(function()
                ...
                if targetItem and targetItemModel then
                    ...
                    -- CRITICAL FIX: Disable collision BEFORE teleport to clip through walls
                    log("11A. Disabling collision to clip through obstacles...")
                    ...
                    log("11B. Collision disabled")
                    
                    -- Teleport DIRECTLY to item position (not above) for better proximity
                    myRoot.CFrame = _CFrame_new(targetItem.Position)
                    
                    -- Immediately point camera at item after teleport
                    log("11C. Pointing camera at item immediately...")
                    _Camera.CFrame = _CFrame_new(myRoot.Position, targetItem.Position)
                    
                    log("12. Teleport complete, waiting...")
                    _task_wait(0.05) -- Slightly longer wait for collision system
                    ...
                    log("21A. Re-enabling collision...")
                    for part, state in pairs(collisionStates) do
                        if part and part.Parent then
                            part.CanCollide = state
                        end
                    end
                    log("21B. Collision re-enabled")
