...
local function loadUILib()
    local success, result = pcall(function()
        if isfile and readfile and isfile("UILIB.lua") then
    ...
    local loadMethods = {
        function() return loadstring(game:HttpGet(repo .. "UILIB.lua"))() end,
        function() 
            if syn and syn.request then
                local response = syn.request({Url = repo .. "UILIB.lua", Method = "GET"})
        ...
        function()
            if request then
                local response = request({Url = repo .. "UILIB.lua", Method = "GET"})
...
if Drawing and Drawing.new and not isMobileDevice then
    ...
    UILib:CreateNotification({
        Text = "Desktop Drawing API Loaded",
        Duration = 3
    })
else
    ...
    local testMarker = Instance.new("Frame")
    testMarker.Size = UDim2.new(0, 50, 0, 50)
    testMarker.Position = UDim2.new(0.5, -25, 0.1, 0)
    testMarker.BackgroundColor3 = Color3.fromRGB(255, 0, 255)
    testMarker.BorderSizePixel = 3
    testMarker.BorderColor3 = Color3.fromRGB(255, 255, 0)
    testMarker.ZIndex = 2000
    testMarker.Parent = fovGui
    ...
    UILib:CreateNotification({
        Text = "📱 Mobile GUI Circles Created!",
        Duration = 5
    })
end
...
local function updateAimbot()
    ...
    elseif FOVCircleGUI then
        if Config.Aimbot and Config.ShowAimbotFOV then
            ...
            FOVCircleGUI.Position = UDim2.new(0, screenWidth / 2, 0, screenHeight / 2)
    ...
    elseif SilentFOVCircleGUI then
        if Config.SilentAim and Config.ShowSilentAimFOV then
            ...
            local screenWidth = Camera.ViewportSize.X
            local screenHeight = Camera.ViewportSize.Y
            SilentFOVCircleGUI.Position = UDim2.new(0, screenWidth / 2, 0, screenHeight / 2)
