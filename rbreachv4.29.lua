                ...
                if Config.DamageMultiplier ~= 1 then
                    ...
                    if rawget(stats, "BodyDamage") then
                        ...
                        pcall(function()
                            ...
                            print("[DEBUG] BodyDamage:", originalDamageValues.BodyDamage, "→", newDamage, "(x" .. Config.DamageMultiplier .. ")")
                        end)
                    end
...
UILib:CreateToggle(CombatPanel, {
    ...
    Callback = function(value)
        if value then
            ...
            local hasWeapon = character and character:FindFirstChildOfClass("Tool") ~= nil
            
            if not hasWeapon then
                -- No weapon equipped, show notification and prevent enabling
                UILib:CreateNotification({
                    Text = "⚠️ Equip a weapon first to enable Rapid Fire!",
                    Duration = 4
                })
                -- Return false to cancel the toggle
                return false
            end
            
            -- Weapon is equipped, enable rapid fire
...
local function smoothTeleport(targetPosition, stepSize)
    ...
    -- Direct CFrame teleportation (anti-cheat only monitors GUI text!)
    local startPos = myRoot.Position
...
task.spawn(function()
    -- Check if firesignal is supported (some mobile executors don't have it)
    if not _firesignal or type(_firesignal) ~= "function" then
        warn("[Retro Breach] Infinite stamina not supported on this executor (firesignal missing)")
    ...
    while true do
        local success, err = _pcall(function()
            ...
            if not restoreStamina or not restoreStamina.OnClientEvent then 
                _task_wait(1)
                return 
            end
            
            -- Extra pcall for firesignal call
            local fireSuccess = _pcall(_firesignal, restoreStamina.OnClientEvent)
            if not fireSuccess then
                -- If firesignal fails, disable to prevent crashes
                Config.InfiniteStamina = false
                warn("[Retro Breach] Infinite stamina disabled due to executor incompatibility")
            end
        end)
        
        if not success then
            warn("[Retro Breach] Infinite stamina error:", err)
        end
