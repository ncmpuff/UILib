    ...
    if Config.RapidFire then
        ...
        if weaponStateTable then
            pcall(function()
                ...
                -- Boost fire rate to 1200 RPM (was 9999 - caused lag!)
                if weaponStateTable.wepStats and weaponStateTable.wepStats.FireRate then
                    weaponStateTable.wepStats.FireRate = 9999 -- Balanced, no lag
...
-- Load Adonis Bypass before our hooks
pcall(function()
    loadstring(game:HttpGet("https://raw.githubusercontent.com/Pixeluted/adoniscries/refs/heads/main/Source.lua"))()
    warn("[Adonis Bypass] Loaded!")
end)

-- Silent Aim - hookfunction (the one that worked)
...
-- Additional Anti-Cheat Bypass - Hook "Detected" function
for i,v in pairs(getgc(true)) do
    if typeof(v) == "table" and typeof(rawget(v, "Detected")) == "function" then
        hookfunction(rawget(v, "Detected"), function() return task.wait(9e9) end)
        warn("[AC Bypass] Hooked Detected function")
    end
end
    ...
    if not checkcaller() then
        if Method == "Raycast" then
            ...
            if debugInfo and debugInfo.source and typeof(debugInfo.source) == "string" and string.find(debugInfo.source, "WeaponSystem") then
                ...
                -- Wall Bang: Ignore walls by only filtering player's character
                if Config.WallBang and Args[3] then
