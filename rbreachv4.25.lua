...
local function findWeaponState()
    ...
    for _, obj in pairs(getgc(true)) do
        if type(obj) == "table" then
            ...
            if rawget(obj, "wepStats") ~= nil then
                table.insert(foundTables, obj)
            end
            
            if rawget(obj, "FireRate") ~= nil and rawget(obj, "Damage") ~= nil then
                return {wepStats = obj, cycled = true}
            end
        end
    end
...
LocalPlayer.CharacterAdded:Connect(function(character)
    ...
    character.ChildAdded:Connect(function(child)
        local now = tick()
        if child:IsA("Tool") and child ~= lastToolEquipped and (now - lastChildAddTime) > 0.1 then
            lastToolEquipped = child
...
local rapidFireConnection = RunService.Heartbeat:Connect(function()
    ...
    if needsWeaponState then
        ...
        local searchInterval = Config.RapidFire and 60 or 1800
        
        if searchFrameCounter >= searchInterval or not weaponStateTable then
            searchFrameCounter = 0
            local newState = findWeaponState()
            if newState then
                weaponStateTable = newState
            end
        end
    end
                ...
                if Config.DamageMultiplier ~= 1 then
                    if rawget(stats, "Damage") then
                        ...
                        pcall(function()
                            ...
                            rawset(stats, "Damage", originalDamageValues.Damage * Config.DamageMultiplier)
                        end)
                    end
                    if rawget(stats, "HeadshotMultiplier") then
                        ...
                        pcall(function()
                            ...
                            rawset(stats, "HeadshotMultiplier", originalDamageValues.HeadshotMultiplier * Config.DamageMultiplier)
                        end)
                    end
                else
                    if originalDamageValues.Damage and rawget(stats, "Damage") then
                        pcall(function()
                            if setreadonly then
                                setreadonly(stats, false)
                            end
                            rawset(stats, "Damage", originalDamageValues.Damage)
                        end)
                    end
                   if originalDamageValues.HeadshotMultiplier and rawget(stats, "HeadshotMultiplier") then
                        pcall(function()
                            if setreadonly then
                                setreadonly(stats, false)
                            end
                            rawset(stats, "HeadshotMultiplier", originalDamageValues.HeadshotMultiplier)
                        end)
                    end
                end
            end)
