...
LocalPlayer.CharacterAdded:Connect(function(character)
    ...
    character.ChildAdded:Connect(function(child)
        local now = tick()
        if child:IsA("Tool") and child ~= lastToolEquipped and (now - lastChildAddTime) > 0.5 then
...
local rapidFireConnection = RunService.Heartbeat:Connect(function()
    ...
    if needsWeaponState then
        ...
        if searchFrameCounter >= 1800 or not weaponStateTable then
            searchFrameCounter = 0
    ...
    if needsWeaponState and weaponStateTable then
        local validState = pcall(function()
            return weaponStateTable.wepStats ~= nil
        end)
...
UILib:CreateButton(CombatPanel, {
    ...
    Callback = function()
        ...
        if found then
            UILib:CreateNotification({
                Text = "✅ Weapon Found - Rapid Fire Ready!",
                Duration = 3
            })
        else
            UILib:CreateNotification({
                Text = "❌ No weapon detected - Try:\n1. Equip a gun\n2. Fire once\n3. Click fix again",
                Duration = 5
            })
        end
    end
