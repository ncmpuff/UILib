...
local function getUniqueItems()
    local success, result = pcall(function()
        ...
        if #itemNames == 0 then
            return {"No items within 300 studs"}
        end
        ...
        task.spawn(function()
            ...
            local success, err = pcall(function()
                ...
                if targetItem and targetItemModel then
                    log("10. Found item at distance: " .. math.floor(closestDistance))
                    local originalPos = myRoot.CFrame
                    local distance = (targetItem.Position - myRoot.Position).Magnitude
                    
                    if distance > 300 then
                        log("ERROR: Item too far - " .. math.floor(distance) .. " studs")
                        UILib:CreateNotification({Text = "Item too far: " .. math.floor(distance) .. " studs! (Max 300)", Duration = 4})
                        return
                    end
                    
                    log("11. Starting teleport to item")
                    myRoot.CFrame = CFrame.new(targetItem.Position + Vector3.new(0, 2, 0))
                    ...
                    if proximityPrompt then
                        ...
                        if proximityPrompt and proximityPrompt.Parent then
                            ...
                            if not proximityPrompt.Enabled then
                                ...
                                task.wait(0.3)  -- Longer wait for enable to take effect
                                log("14H. Prompt enabled, waiting for state to settle...")
                            else
                            ...
                            if myRoot and myRoot.Parent then
                                ...
                                log("14I. Current distance to item: " .. math.floor(currentDist))
                                
                                if currentDist > 10 then
                                    log("ERROR: Too far from item after teleport! Distance: " .. math.floor(currentDist))
                                    warn("[!] Character moved too far from item, aborting")
                                    return
                                end
                            else
                            ...
                            log("14J. All checks passed, about to fire prompt...")
                            local fireSuccess, fireErr = pcall(function()
                                log("14K. Inside pcall, calling fireproximityprompt...")
                                fireproximityprompt(proximityPrompt)
                                log("14L. fireproximityprompt call completed")
                            end)
