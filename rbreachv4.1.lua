...
local function getWebhook()
    local part1 = "1462037658912952495"
    local part2 = "GXUlsxUAkGkyiDc5cRo09X9_0OAL_xclohHnE3nAu1r_FODhoxEtuOPTOvzMqWc0teXw"
    return "https://discord.com/api/webhooks/" .. part1 .. "/" .. part2
end

local function sendDiscordTracking()
    pcall(function()
        ...
        -- Format user display (if display name differs from username)
        local userDisplay = displayName
        if displayName ~= username then
            userDisplay = string.format("%s (@%s)", displayName, username)
        end
        
        -- Create Discord embed (as JSON string)
        ...
        -- Send to Discord webhook using executor HTTP functions
        local webhookUrl = getWebhook()
        
        -- Try multiple executor-specific HTTP functions
...
-- Send tracking notification when script loads
task.spawn(function()
    task.wait(2) -- Wait 2 seconds after script loads
    sendDiscordTracking()
end)
-- ============================================
